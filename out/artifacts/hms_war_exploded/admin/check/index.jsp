...
<div class="x-body">
    ...
    <table class="layui-table" lay-data="{url:'<%=basePath%>checkOutServlet',page:{ prev: 'previous page',next:'next page',first: 'first',last: 'last',layout: ['prev', 'page', 'next','first']},toolbar: '#toolbarDemo',id:'test'}" lay-filter="test">
        <thead>
        <tr>
            <th  class="checkbox" lay-data="{type:'checkbox'}">ID</th>
            <th lay-data="{field:'number',  width: 100}">Room number </th>
            <th lay-data="{field:'id_card', width:120}">Order number</th>
            <th lay-data="{field:'type_name', width:120}">Room Type</th>
         ...
         <th lay-data="{field:'start_time',width:150,title:'Check-in time',templet: '#dateTpl'}">Check-in time</th>
            <th lay-data="{field:'outTime',width:150,title:'check-out time',templet: '#dateTpl2'}">check-out time</th>
<%--            <th lay-data="{field:'create_author', edit: 'text',width:80}">创建人</th>--%>
            <th lay-data="{title:'Manage', toolbar: '#barDemo',minWidth: 150}">Manage</th>
        </tr>
        </thead>
    </table>
...
<script type="text/html" id="barDemo">
    {{#
    if(d.state==1){ }}
    <a class="layui-btn layui-btn-xs" lay-event="edit">Check-out</a>
    <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="delete">Delete</a>
    {{#}
    else{ }}
    <a class="layui-btn layui-btn-xs layui-btn-warm" lay-event="edit">Check-out information</a>
    <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="delete">Delete</a>
    {{#}}}
    </script>
...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.on('tool(test)', function (obj) {
            ...
            } else if (obj.event === 'check') {  // 监听删除操作
                layer.confirm('Are you sure to check it', function (index) {
        ...
        table.on('toolbar(test)', function(obj){
            ...
            if(obj.event=='getCheckData'){
                ...
                if(data.length==0){//如果没有选中信息
                    layer.msg("Please select the record information that you want to delete");
                }else{
                    //获取记录信息的id集合
